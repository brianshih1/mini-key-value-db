...
use super::latch_manager::LatchGuard;
...
type AddStack<'a, K: NodeKey> = Vec<(Option<usize>, Option<RwLockWriteGuard<'a, Node<K>>>)>;
type DeleteStack<'a, K: NodeKey> = Vec<(usize, Direction, RwLockWriteGuard<'a, Node<K>>)>;
...
impl<K: NodeKey> InternalNode<K> {
    ...
    pub fn split_node(&self) -> (LatchNode<K>, K) {
        //
...
impl<K: NodeKey> LeafNode<K> {
    ...
    pub fn split_node(&self, latch_node: LatchNode<K>) -> (LatchNode<K>, K) {
        ...
        let new_right_node = LeafNode {
            ...
            left_ptr: RefCell::new(Some(Arc::downgrade(&latch_node))),
            right_ptr: RefCell::new(right_sibling),
            order: self.order,
        };
        let right_arc = Arc::new(RwLock::new(Node::Leaf(new_right_node)));
        self.right_ptr
            .borrow_mut()
            .replace(Arc::downgrade(&right_arc));
        (right_arc, right_start)
    }
    ...
    pub fn steal_from_right_leaf_sibling(
        ...
        if right_sibling.has_spare_key() {
            ...
            for (iter_idx, (idx, direction, write_guard)) in stack.iter().enumerate() {
                let internal_node = write_guard.as_internal_node();
    ...
    pub fn steal_from_left_leaf_sibling(
        ...
        if left_sibling.has_spare_key() {
            ...
            let (idx, direction, write_guard) = &stack[stack.len() - 1];
            let parent_node = write_guard.as_internal_node();
     ...
     */
    pub fn delete_key(&self, key_to_delete: &K, stack: &DeleteStack<K>) -> bool {
        ...
        let (edge_idx, _, write_guard) = &stack[stack.len() - 1];
        let parent_node = write_guard.as_internal_node();
...
impl<K: NodeKey> BTree<K> {
     ...
     */
    pub fn find_leaf_to_delete(&self, key_to_delete: &K) -> (Option<LatchNode<K>>, DeleteStack<K>) {
        let mut temp_node = self.root.borrow().clone();

        let mut next = None;
        let mut stack = Vec::new();
        loop {
            if let Some(ref node) = temp_node {
    ...
    pub fn find_leaf_to_add<'a>(&self, key_to_add: &K) -> AddStack<K> {
        let mut temp_node = self.root.borrow().clone();
        ...
        let mut stack: AddStack<K> = Vec::new();
        loop {
            if let Some(ref latch_node) = temp_node {
                let foo = latch_node.as_ref();
                let write_guard = foo.write().unwrap();
                match &*write_guard {
                    Node::Internal(internal_node) => {
    ...
    pub fn insert(&self, range: Range<K>) -> () {
        ...
        let mut parent_stack: AddStack<K> = Vec::new();
        loop {
            if let Some(ref latch_node) = temp_node {
                let foo = latch_node.as_ref();
                let write_guard = foo.write().unwrap();
            ...
            parent_stack.push((last_idx, last_guard));
        ...
        let leaf_arc = leaf.unwrap();
        let write_guard = leaf_arc.write().unwrap();
        match *write_guard {
            ...
            Node::Leaf(leaf_node) => {
                ...
                if !leaf_node.has_capacity() {
                    let (mut split_node, mut median) = leaf_node.split_node(leaf_arc.clone());

                    let mut offset = 0;
                    let mut current_node = Some(write_guard);
                    loop {
                        if let Some(current_guard) = current_node {
                            if parent_stack.len() - offset > 0 {
                                let idx = parent_stack.len() - 1 - offset;
                                let (edge_idx, node) = parent_stack[idx];
                                current_node = node;
                                // this is the node we want to insert the

                                let curr_parent = current_guard.as_internal_node();
                                let latch_node = curr_parent.edges.borrow()[edge_idx.unwrap()]
                                    .borrow()
                                    .clone()
                                    .unwrap();
                                ...
                                (split_node, median) = match &*current_guard {
                                    Node::Internal(ref internal) => internal.split_node(),
                                    Node::Leaf(ref leaf) => leaf.split_node(latch_node),
                                };
                                offset = offset + 1;
                            } else {
                                ...
                                self.root.borrow_mut().replace(Arc::new(RwLock::new(
                                    Node::Internal(InternalNode {
                                        ...
                                        edges: RefCell::new(Vec::from([
                                            RefCell::new(self.root.borrow().clone()),
                            ...
                            // once we see a null, then this means we won't need to update parents as the write lock is released
                            return;
                        }
                    }
                }
        ...
        if let Some(ref node_ref) = node_to_delete {
            ...
            if did_leaf_merge {
                ...
                loop {
                    ...
                    let (_, _, write_guard) = &stack[i];
                    let internal_node = write_guard.as_internal_node();
                    let (edge_idx, _, parent_guard) = &stack[i - 1];
                    let parent_node = parent_guard.as_internal_node();
                    internal_node.deal_with_underflow(parent_node, *edge_idx);
            ...
            if stack.len() > 1 {
                let (_, _, root_guard) = &stack[0];
                let root_internal_node = root_guard.as_internal_node();
                if root_internal_node.keys.borrow().len() == 0 {
                    let new_root = root_internal_node.edges.borrow()[0]
                        .borrow()
                        .clone()
                        .unwrap();
...
//         tree: &BTree<K>,
//         indices: Vec<usize>,
//     ) -> (Option<LatchNode<K>>, Vec<LatchNode<K>>) {
...
//             match temp_node {
//                 Some(ref node) => {
//                     let guard = node.as_ref().read().unwrap();
//                     match &*guard {
//                         Node::Internal(internal_node) => {
...
//                 (leaf_latch, Vec::from([leaf_latch.clone()]))
//             }
//         }
//     }
...
//     pub fn print_node<K: NodeKey>(node: LatchNode<K>) {
//         let guard = node.as_ref().read().unwrap();
//         match &*guard {
//             Node::Internal(node) => {
...
//         if let Some(ref weak_latch) = edge {
//             let upgraded_ref = weak_latch.upgrade();
//             let unwrapped = upgraded_ref.unwrap().as_ref().read().unwrap();
//             match *unwrapped {
//                 Node::Internal(_) => {
...
//             let unwrapped = upgraded_ref.as_ref().read().unwrap();
//             match *unwrapped {
//                 Node::Internal(_) => {
//                     panic!("Cannot get sibling from internal node");
//                 }
...
//         if let Some(ref weak_latch) = edge {
//             let node = weak_latch.read().unwrap();
//             match *node {
...
//             let left_sibling = &*guard.as_leaf_node().left_ptr.borrow();
//             let right_sibling = &*guard.as_leaf_node().right_ptr.borrow();
//             if idx == 0 {
...
//                 let guard = node.clone().as_ref().read().unwrap();
...
//     fn get_all_leaf_nodes<K: NodeKey>(latch_node: LatchNode<K>) -> Vec<LatchNode<K>> {
//         let mut leaves = Vec::new();
//         let guard = latch_node.read().unwrap();
//         match *guard {
//             Node::Internal(internal_node) => {
//                 for edge in internal_node.edges.borrow().iter() {
...
//     fn assert_internal<K: NodeKey>(internal_node: &InternalNode<K>, start_keys: Vec<K>) {
...
//             assert_internal(
//                 stack[0].1.unwrap().as_internal_node(),
//                 Vec::from([12, 15, 19]),
//             );
...
//             let (split_node, median) = read_guard.as_internal_node().split_node();
//             assert_eq!(median, 20);
...
//         #[test]
//         fn split_leaf() {
//             // TODO: Rewrite this test with TestNode
//             // let leaf = LeafNode {
...
//             // let leaf_latch_node = Arc::new(RwLock::new(Node::Leaf(leaf)));
//             // let right_sibling = LeafNode {
//             //     start_keys: RefCell::new(Vec::from([4, 5, 6])),
//             //     end_keys: RefCell::new(Vec::from([0, 1, 2])),
//             //     left_ptr: RefCell::new(Some(Arc::downgrade(&leaf_latch_node))),
//             //     right_ptr: RefCell::new(None),
//             //     order: 4,
//             // };
//             // let right_sibling_rc = Arc::new(RwLock::new(Node::Leaf(right_sibling)));
//             // match leaf_latch_node.as_ref() {
...
//             // let (split_node, right_start_key) = BTree::split_node(leaf_latch_node.clone());
...
//                 });
//                 let tree = create_test_tree(&test_node, 3);
//                 let (node_latch, parent, edge_idx) =
//                     find_node_and_parent_with_indices(&tree, Vec::from([1]));
//                 let node_guard = node_latch.as_ref().read().unwrap();
//                 let temp = node_guard.as_internal_node();
//                 let parent_guard = parent.as_ref().read().unwrap();
...
//                 let parent_guard = parent_latch.as_ref().read().unwrap();
//                 let parent = parent_guard.as_internal_node();
...
//                 let (node_latch, parent_latch, edge_idx) =
//                     find_node_and_parent_with_indices(&tree, Vec::from([1]));
//                 let node_guard = node_latch.read().unwrap();
...
//                 let guard = node.unwrap().as_ref().read().unwrap();
